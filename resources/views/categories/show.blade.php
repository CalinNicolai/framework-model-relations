...
@section('content')
    ...
    @if($category->parent)
        <a class="border-2 m-5 p-2 rounded border-black bg-red-300" href="{{ route('categories.show', $category->parent->id) }}">{{ $category->parent->name }}</a>
    @endif
    @foreach($category->children as $child)
        <a class="border-2 m-5 p-2 rounded border-black bg-green-600" href="{{ route('categories.show', $child->id) }}">{{ $child->name }}</a>
    @endforeach
